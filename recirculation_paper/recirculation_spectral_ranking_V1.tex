...
\title{Recirculation: Spectral Ranking of Journals and Organisations in Economic and Business Science}
\date{Technical Report, September 27, 2025}
...
\subsection{Social Systems Theory of Citation}

\subsection{Citation Matrices}

\cite{pinski} define the citation matrix $C_{ij}$ as a table of \textit{references}  from unit $i$ to unit $j$ or equally of \textit{citations} to unit $j$ from unit $i$. We follow this convention with $i, j$ denoting rows, columns respectively, and note that the transposed convention is also frequently adopted \cite[e.g. ][]{prathapPinskiNarinInfluence2019}.

In this paper we consider individual research articles (papers) aggregated into journal sets that frame specified topics. Each paper has an affiliation list comprising authors and their institutions, as well references to earlier works. These establish directional reference/citation links between papers, and hence journals and institutions. When aggregated, these links become weights in the citation matrix. We select a window of publication years for the referencing papers (publication window) and for referenced works (census window). Reference lists will include works that do not lie in the census window or the referencing journal set and institution set. These works are omitted.

There are many ways to assemble article aggregations. We adopt the viewpoint that a single reference has unit value to be divided among referencing paths. A weight of 0.5 is allocated to the path from referencing to cited journal, while the other 0.5 is allocated equally across the paths between institutions. If the referencing work has $N^r$ distinct institutions and the cited work $N^c$ institutions, each distinct institution/institution path including self-paths receives a weight of $0.5/(N^r \times N^c)$. These rules lead to a citation matrix that has two blocks, one for journals and one for institutions. The sum of elements in both blocks is one-half the number of references to included journals and institutions  Rather than include mixed journal/institution paths in the citation matrix we recirculate between the blocks as described below.        

\subsection{Measures of Influence and Spectral Ranking}

A librarian managing the cost of scholarly journal subscriptions might well choose journals that attract the greatest number of citations in a field.  
