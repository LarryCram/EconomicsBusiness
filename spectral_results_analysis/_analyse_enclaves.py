...
def cluster_hcw_infomap(df_works: pd.DataFrame,
    ...
    raw_scc = sccs.membership
    scc_size_rank = {cid: rank for rank, (cid, _) in enumerate(
        sorted(Counter(raw_scc).items(), key=lambda x: -x[1])
    )}
    hcw['scc_id'] = [scc_size_rank[raw_scc[id_to_idx[w]]] for w in hcw['work_idx']]

    total_sccs  = len(set(raw_scc))
    n_singleton = sum(1 for s in Counter(raw_scc).values() if s == 1)
    print(f'\n── Strongly connected components  '
          f'({total_sccs} total, {n_singleton} singletons, showing top 10) ──')
    print(f'{"#":>4} {"n_hcw":>6} {"med_v":>6} {"fld":>4}  {"Top terms":<50}')
    print('-' * 78)
    for scc_rank in range(min(10, total_sccs)):
        grp = hcw[hcw['scc_id'] == scc_rank]
        if grp.empty:
            continue
        ...
        print(
            f"{scc_rank:>4} {len(grp):>6} "
            f"{grp['v'].median():>6.2f} {dom_field:>4}  "
            f"{top_label[:50]}"
        )
    ...
    print(f'\n── Infomap communities  ({total_comm} total, {len(df_comm)} with ≥{min_community} HCW,  showing top 10) ──')
    print(f'{"#":>4} {"n_hcw":>6} {"med_v":>6} {"fld":>4}  {"Top terms":<50}')
    print('-' * 78)
    for _, r in df_comm.head(10).iterrows():
        print(
    ...
    low_v_comms = df_comm.head(10)[df_comm.head(10)['median_v'] < LOW_V_THRESH]['community_id'].tolist()
    if low_v_comms:
    ...
    # ── top-25 E + top-25 B sources by v: HCW in top-10 communities
    src_meta = (
        hcw[['source_idx', 'source_name', 'v', 'field_eb']]
        .drop_duplicates('source_idx')
    )
    top25_src = pd.concat([
        src_meta[src_meta['field_eb'] == f].nlargest(25, 'v')
        for f in ['E', 'B']
    ]).drop_duplicates('source_idx')
    ...
    print(f'\n── Top-25 E/B sources by v — HCW counts in top-10 communities ──────────')
