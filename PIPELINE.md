...
`field_eb` ∈ {E, B, A, null} plus `era_field`, `harzing_field`, `wos_categories`.
Git-tracked; consumed by edge building and all fig scripts.
...
**Corpus configurations built** (driven by `params.csv`):

| run_code | fx | tau_u | tau_s | Purpose |
|---|---|---|---|---|
| 20242024 | A, E, B, EB, NEB | 20 | 20 | Baseline period × field subsets |
| 20242024 | A | 40 | 40 | τ sensitivity only |
| 00040004–15191519 | A | 20 | 20 | Time-series runs t1–t4 |
...
**Run schedule** (all cases needed for the paper; driven by `params.csv`):

### Phase 1 and Phase 2 runs (run_code=20242024)

| Label | run_code | fx | tau_u | tau_s | rho | m | chi | alpha | Used by |
|---|---|---|---|---|---|---|---|---|---|
| `baseline` | 20242024 | A | 20 | 20 | 0 | 0110 | 0.50 | 1.0 | all figs |
| `F=E` | 20242024 | E | 20 | 20 | 0 | 0110 | 0.50 | 1.0 | fig_2 |
| `F=B` | 20242024 | B | 20 | 20 | 0 | 0110 | 0.50 | 1.0 | fig_2 |
| `F=EB` | 20242024 | EB | 20 | 20 | 0 | 0110 | 0.50 | 1.0 | fig_2 |
| `SS-only` | 20242024 | A | 20 | 20 | 0 | 1000 | 0.50 | 1.0 | fig_3 |
| `II-only` | 20242024 | A | 20 | 20 | 0 | 0001 | 0.50 | 1.0 | fig_3 |
| `full-joint-chi-star` | 20242024 | A | 20 | 20 | 0 | 1111 | χ* | 1.0 | fig_3 |
| `tau40` | 20242024 | A | 40 | 40 | 0 | 0110 | 0.50 | 1.0 | fig_5 |
| `rho1` | 20242024 | A | 20 | 20 | 1 | 0110 | 0.50 | 1.0 | fig_5 |

χ* is computed at runtime as N_u/(N_s+N_u) from the baseline units table.

### Phase 4 time-series runs

| Label | run_code | fx | tau_u | tau_s | rho | m | chi | alpha |
|---|---|---|---|---|---|---|---|---|
| `t1` | 00040004 | A | 20 | 20 | 0 | 0110 | 0.50 | 1.0 |
| `t2` | 05090509 | A | 20 | 20 | 0 | 0110 | 0.50 | 1.0 |
| `t3` | 10141014 | A | 20 | 20 | 0 | 0110 | 0.50 | 1.0 |
| `t4` | 15191519 | A | 20 | 20 | 0 | 0110 | 0.50 | 1.0 |

run_code=20242024 is the baseline (2020–24).  Time-series runs are identified by label (t1–t4), not by a stage number.
...
| `fig_5.py` | `rankings.duckdb` | Phase 2 sensitivity (τ, ρ) |
| `fig_6.py` | `rankings.duckdb` | Time-series comparison (t1–t4 vs baseline 2020–24) |
| `fig_7.py` | `bootstrap/*.npy` + `rankings.duckdb` | Bootstrap uncertainty |
...
2. **`fig_5.py`** — currently calls `build_csr` + `bipartite()` on-the-fly.
   Fix: extend `rank()` to accept `mu`, add `alpha05-mublock` to `params.csv`,
...
### `params.yaml`
Controls corpus construction and sensitivity values.
...
### `params.csv`
Run schedule for `run_rankings.py`.  One row per run.

Columns: `skip`, `run_code`, `tc0`, `tc1`, `tt0`, `tt1`, `fx`, `tau_u`, `tau_s`,
`rho`, `m`, `chi`, `alpha`, `mu_type`, `label`.

`run_code` is an 8-character string encoding the last two digits of tc0, tc1, tt0, tt1
(e.g. `20242024` for the 2020–24 baseline, `00040004` for t1 2000–04).
`chi=-1` signals χ* (resolved at runtime).
`skip=1` rows are ignored by `load_runs()` and `runs_from_csv()`.
