...
For the bipartite mode, the one-mode projection $M_S = H_{SI} H_{IS}$ represents a two-hop walk $S \to I \to S$, so the damping applied to $M_S$ in power iteration is $\alpha^2$, ensuring that the per-hop attenuation is $\alpha$ in all modes and that $v_s$ scores are directly comparable across the source-only and bipartite rankings at the same $\alpha$.
