...
\begin{itemize}
	...
	\item For runs with $\alpha=1$, $\boldsymbol{\mu}=\mathbf{0}$ including the baseline, the dominant eigenpair of $\mathbf{\tilde{H}}^\top$ is computed with \texttt{scipy.sparse.linalg.eigs}, which also returns the next few eigenvalues and thereby verifies primitivity (a unique eigenvalue of modulus~1).
