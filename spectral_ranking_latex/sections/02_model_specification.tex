...
institutions of author $\ell$. Using the work-institution frequency distribution to determine a minimum cut for works per institution per year $\geq\tau_U$, we retain the set $\mathfrak{U}$ and the corresponding set of authors $\mathfrak{A}$. Since $\omega_{iu}$ requires both an author and an institution, we drop any authorship where the institution $\notin\mathfrak{U}$, and subsequently drop works that lose all their authorships. The institution weights are not renormalised to account for the omission of an authorship, so that $\sum_u\omega_{iu}\leq 1$.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{lllX}
		...
		$[t^c_0 \ldots t^c_1]$   & census window         & interval        & publication years of works \newline \hspace*{1em} whose references are traced \\
		$[t^t_0 \ldots t^t_1]$   & target window         & interval        & publication years of works \newline \hspace*{1em} eligible to receive references \\
		$\tau_U$                 & institution threshold & integer         & minimum work count to retain an institution \\[4pt]
		\multicolumn{4}{@{}l@{}}{\textit{Matrix construction}} \\[2pt]
		$\delta\in\{0,1\}$       & reference normalisation & binary        & 0: a work  contributes $\bar{R}$ units \newline 1: a work contributes $R$ units \\
...
with entries $\mu_p = a_p / \sum_q a_q$,
