...
retained institutions of author $\ell$, so that $\sum_u\omega_{iu}=1$. Re-weighting fractional counts as a consequence of pruning the number of institutions is a trade-off between boosting the residual institutions and diminishing the total contribution of a work. 
...
Table \ref{tab:params} sets out the parameters of the model. Note that the corpus excludes (a) census work references not giving attention to  target works, (b) non-census works giving attention to target works, and (c) authorships that include an omitted institution.
...
Let $\boldsymbol{\mu}$ be a prior (boundary condition;
...
there; the prior below then supplies the necessary probability
mass.
...
\begin{equation}
    \mathbf{v}(\alpha)
    \;=\;
    A\,\mathbf{A}^{-1}\boldsymbol{\pi}(\alpha),
    \qquad
...
A unit with $v_p=1$ therefore has exactly the corpus-average prestige per work;
$v_p=2$ indicates twice the average.
...
Note that $A\,\mathbf{A}^{-1}$ must be applied after convergence; applying
...
forming a directed bipartite graph between sources and institutions.
