...
With $\mathbf{m}=(0110)$, equation~\eqref{eq:C} yields block off-diagonal $\mathbf{C}$ and $\mathbf{H}$,
...
Because every path alternates $S\to I\to S\to\cdots$, this structure makes $\mathbf{H}$ periodic with
period~2 and the undamped eigenvector equation~\eqref{eq:eig} has no unique solution.
The modification $\tilde{\mathbf{H}}$ in equation~\eqref{eq:Hstar} restores primitivity for
any $\alpha\in(0,1)$.
...
\]
Both $\mathbf{M}_S$ and $\mathbf{M}_I$ are row-stochastic (as products of row-stochastic matrices).
