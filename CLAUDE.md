...
```
Edge lists:   el_{run_code}_{fx}_tauU{tau_u}_tauS{tau_s}
Units index: _units_{run_code}_{fx}_tauU{tau_u}_tauS{tau_s}
Rankings:    rk_{run_code}_{fx}_tauU{tau_u}_tauS{tau_s}_rho{rho}_m{m}_chi{chi_str}_alpha{alpha_int}
```
where `chi_str='STAR'` when chi==-1.0, otherwise `str(round(chi*100))` (e.g. 0.5→'50').
