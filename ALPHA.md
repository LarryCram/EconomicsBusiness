...
- call power_iteration(M_S, alpha, mu_eff) for pi_S
  - at alpha=1, mu=None: mu_eff = None
  - at alpha<1, mu!=None: mu_eff = mu_S + sqrt(alpha) * H_IS.T @ mu_I  (effective prior)
- recover pi_I:
  - at alpha=1: pi_I = H_SI.T @ pi_S
  - at alpha<1, mu!=None: pi_I = sqrt(alpha) * H_SI.T @ pi_S + (1-sqrt(alpha)) * mu_I
