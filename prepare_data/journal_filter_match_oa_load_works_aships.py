...
def filter_and_match(db):
    ...
    db.sql(sql)
    sql = """
        SELECT count(source_id) AS source_count, subfield
...
def load_works_authorships(db):
    sql = """
        ...
        SELECT w.*
                FROM read_parquet('/home/lc/m/working/econ_bus/parquet/sources.parquet')
                JOIN (SELECT * FROM read_parquet('/home/lc/m/openalex_feb26/parquet/works/*.parquet')) w
                USING (source_id)
        ...
        SELECT a.*
            ...
            JOIN (SELECT * FROM read_parquet('/home/lc/m/openalex_feb26/parquet/authorships/*.parquet')) a
            USING (work_id)
