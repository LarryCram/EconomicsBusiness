...
def main():
    ...
    # Compute PageRank on transpose with alpha=1.0
    pr_t = pagerank(C.T, alpha=1.0)
    ...
    print('\nComparing PageRank(C.T, alpha=1.0) to Geller(predicted)')
    print('Unit\tPageRank_T\tGeller\tDiff')
    print('-'*60)
    for u in units:
        pr_val = pr_t['pi'].get(u, 0.0)
        g_val = geller['pi'].get(u, 0.0)
        diff = pr_val - g_val
        print(f"{u}\t{pr_val:.6f}\t{g_val:.6f}\t{diff:.6e}")

    import numpy as np
    pr_vec = [pr_t['pi'][u] for u in units]
    g_vec = [geller['pi'][u] for u in units]
    l1 = np.sum(np.abs(np.array(pr_vec) - np.array(g_vec)))
    print('\nL1 norm of difference:', l1)

    print('\nTop 5 PageRank (C.T):')
    for k, v in fmt_sorted(pr_t['pi'])[:5]:
        print(f"  {k}: {v:.6f}")
